		...
		axios
			...
			.then((response) => {
				...
				data.products.forEach((product) => {
					...
					const recLabels = { 1: 'Similar Albums', 2: 'Similar CDs', 3: 'Similar Items' };
					product.recLabel = recLabels[product.categoryId] || 'Similar Albums';
				});
...
async function getRecommendations(productId) {
	try {
		const response = await axios.get(
			`${config.baseUrl}/products/${productId}/recommendations`,
		);
